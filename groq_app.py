...
def groq_chat(messages, model: str = GROQ_MODEL, temperature: float = 0.2) -> str:
...
"""
    text = groq_chat([{"role": "user", "content": prompt}], temperature=0.0)
...
def retrieve_with_multiquery(vectordb, question: str, k_per_query: int = 5, max_docs: int = 10):
...
Two help modes:
- training: If user has asked specific qustion, answer is less than 100 words. Else teach step-by-step + why it matters + tips to avoid common mistakes.
...
def groq_chat_with_images(system_prompt: str, user_text: str, image_files: List[Tuple[str, bytes, str]], temperature: float = 0.2) -> str:
